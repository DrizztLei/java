...
public class AttitudeAnalysis
{
	...
	public double query() throws UnsupportedEncodingException
	{
		...
		for (int i = 0, length = Rule.getWord().length; i < length; i++)
		{
			...
			Tool.getPrintWriter()
					.println(Tool.getResult(getPositive().test(Tool.getDoubleMatrix(Tool.changeCode(input))), true)
							+ "\t is the positive .");
			Tool.getPrintWriter()
					.println(Tool.getResult(getNegative().test(Tool.getDoubleMatrix(Tool.changeCode(input))), false)
							+ "\t is the negative .");
