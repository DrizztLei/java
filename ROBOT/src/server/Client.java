...
public class Client implements Runnable
{
	...
	public void run()
	{
		...
		Tool.getPrintWriter().println("Client : The th in ArrayList is "
				+ (MyServer.getSocketList().indexOf(clientSocket) + 1) + " \nClient : The user has been online . ");
			...
			while (true)
			{
				...
				} else
				{
					...
					} else
					{
						...
						{
							new PrintWriter(MyServer.getU3DSocket().get(i).getOutputStream(), true)
									.println(fenci.getOutput());
						}
		...
		} catch (IOException e)
		{
			...
			// MyServer.flag = false;
			// Tool.getPrintWriter().println("Client : IOException and change
			// the flag . ");
			if (MyServer.check)
			{
				MyServer.flag = true;
